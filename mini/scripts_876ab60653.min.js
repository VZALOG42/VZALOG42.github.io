...
$(function(){var percent=8;var startSum=20000;var maxSum=200000;var start=1;var end=12;var endPeriod=12;var type=false;$("#range2").ionRangeSlider({hide_min_max:true,keyboard:true,min:1,max:12,from:1,to:12,type:'single',step:1,grid:false,onChange:function(res){var sum=parseInt($("#range").val());var period=res.from+1;resolveSum(percent,sum,period,type);}});if($("#range2").length>0){var period=1;var arrPeriod=$("#range2").attr('data-values').split(',');for(var i=1;i<=arrPeriod.length;i++){if(arrPeriod[i]==$("#range2").val()){var period=i;}}}
$("#range").ionRangeSlider({hide_min_max:true,keyboard:true,min:20000,max:1000000,from:20000,to:1000000,type:'single',step:10000,postfix:" руб.",grid:false,onChange:function(res){resolveSum(percent,res.from,period,type);}});});Number.prototype.format=function(n,x,s,c){var re='\\d(?=(\\d{'+(x||3)+'})+'+(n>0?'\\D':'$')+')',num=this.toFixed(Math.max(0,~~n));return(c?num.replace('.',c):num).replace(new RegExp(re,'g'),'$&'+(s||','));};function resolveSum(persent,sum,period,type){var returnSum=0,payment=0;if(type){returnSum=Math.round(((persent / 100 / 30*period)*sum)+sum);}
else{returnSum=Math.round(((persent / 100*sum)*period)+sum);}
if(type){payment=Math.round((persent / 100 / 30*sum)*period);}
else{payment=Math.round((persent / 100*sum)*period);}
$('#suuma-calc').html(sum.format(0,3,' '));$("#returnSum").text(returnSum.format(0,3,' '));$("#payment").text(payment.format(0,3,' '));return{"returnSum":returnSum,"payment":payment}}
...
;(function($,window,document,undefined){"use strict";$.fn.chained=function(parentSelector){return this.each(function(){var child=this;var backup=$(child).clone();$(parentSelector).each(function(){$(this).bind("change",function(){updateChildren();});if(!$("option:selected",this).length){$("option",this).first().attr("selected","selected");}
updateChildren();});function updateChildren(){var triggerChange=true;var currentlySelectedValue=$("option:selected",child).val();$(child).html(backup.html());var selected="";$(parentSelector).each(function(){var selectedValue=$("option:selected",this).val();if(selectedValue){if(selected.length>0){selected+="+";}
...
var matches=[];var data=$(this).data("chained");if(data){matches=data.split(" ");}
if((matches.indexOf(selected)>-1)||(matches.indexOf(selectedFirst)>-1)){if($(this).val()===currentlySelectedValue){$(this).prop("selected",true);triggerChange=false;}}else{$(this).remove();}});if(1===$("option",child).length&&$(child).val()===""){$(child).prop("disabled",true);}else{$(child).prop("disabled",false);}
if(triggerChange){$(child).trigger("change");}}});};$.fn.chainedTo=$.fn.chained;$.fn.chained.defaults={};})(window.jQuery||window.Zepto,window,document);
;(function($,window,document,undefined){"use strict";$.fn.remoteChained=function(options){var settings=$.extend({},$.fn.remoteChained.defaults,options);if(settings.loading){settings.clear=true;}
return this.each(function(){var self=this;var request=false;$(settings.parents).each(function(){$(this).bind("change",function(){var data={};$(settings.parents).each(function(){var id=$(this).attr(settings.attribute);var value=($(this).is("select")?$(":selected",this):$(this)).val();data[id]=value;if(settings.depends){$(settings.depends).each(function(){if(self!==this){var id=$(this).attr(settings.attribute);var value=$(this).val();data[id]=value;}});}});if(request&&$.isFunction(request.abort)){request.abort();request=false;}
if(settings.clear){if(settings.loading){build.call(self,{"":settings.loading});}else{$("option",self).remove();}
$(self).trigger("change");}
request=$.getJSON(settings.url,data,function(json){json=settings.data(json);build.call(self,json);$(self).trigger("change");});});if(settings.bootstrap){build.call(self,settings.bootstrap);settings.bootstrap=null;}});function build(json){var selectedKey=$(":selected",self).val();$("option",self).remove();if($.isArray(json)){$.each(json,function(key,value){$.each(value,function(key,value){if("selected"===key){selectedKey=value;}else{var option=$("<option />").val(key).append(value);$(self).append(option);}});});}else{$.each(json,function(key,value){if(json.hasOwnProperty(key)){if("selected"===key){selectedKey=value;}else{var option=$("<option />").val(key).append(value);$(self).append(option);}}});}
$(self).children().each(function(){if($(this).val()===selectedKey+""){$(this).attr("selected","selected");}});if(1===$("option",self).length&&$(self).val()===""){$(self).prop("disabled",true);}else{$(self).prop("disabled",false);}}});};$.fn.remoteChainedTo=$.fn.remoteChained;$.fn.remoteChained.defaults={attribute:"name",depends:null,bootstrap:null,loading:null,clear:false,data:function(json){return json;}};})(window.jQuery||window.Zepto,window,document);
$(document).ready(function(){$("#series").chained("#mark");$('nav > .icon-menu').click(function(event){$(this).toggleClass('active');$(this).next('div.nav_block').toggleClass('active');});$('.nav_block ul li a').on('click',function(){});$(function(){$('ul.tabs.myfaq').delegate('li:not(.current)','click',function(){$(this).addClass('current').siblings().removeClass('current').parents('').find('div.mybox').hide().eq($(this).index()).fadeIn(150);})})
$('.question').on('click',function(){if($(this).next().is(":hidden")){$(this).next().slideDown('slow');}else{$(this).next().slideUp('slow');}
return false;});if(window.location.hash=="#pol"){$('#pol').trigger('click');}
if(window.location.hash=="#ver"){$('#ver').trigger('click');}if(window.location.hash=="#prod"){$('#prod').trigger('click');}
if($(window.location.hash).length>0){$('html, body').animate({scrollTop:$(window.location.hash).offset().top},1000);}
$('.examples_loans > span, .news_promotions > span').on('click',function(){$('div.examples_loans').toggleClass("active");$('div.news_promotions').toggleClass("active");});if($('input[type="checkbox"]').length){$('input[type="checkbox"]').each(function(){$(this).after('<label for="'+$(this).attr('id')+'" class="checkbox"></label>');$(this).hide();});}});$(document).ready(function(){var owl=$('.rewievs_slider');owl.owlCarousel({margin:0,nav:false,loop:true,autoplay:false,autoplayTimeout:5000,items:1,autoHeight:true})
var owl=$('.slider_hidden');owl.owlCarousel({margin:0,nav:false,loop:true,autoplay:false,autoplayTimeout:5000,items:1})
$("#maskMoney").maskMoney({thousands:" ",precision:0,suffix:" рублей"});$("#maskMoney").maskMoney('mask');$('select').styler({'selectVisibleOptions':'10',onSelectClosed:function(){$('select').trigger('refresh');}});$("input[name=phone]").mask("+7 (999)-999-99-99");$(".fancybox").fancybox({'autoResize':true,'openEffect':'drop','closeEffect':'drop','speedIn':600,'speedOut':200,'overlayShow':true,'padding':0,'autoCenter':true,'autoHeight':false,'autoWidth':false,'scrolling':"no"});})
$(function(){$('input[placeholder], textarea[placeholder]').placeholder();});function moreStep(step){if(step=="step2"){$('.steps li').removeClass('active');$('.step2li').addClass('active');$('.step1').css('display','none');$('.step2').css('display','block');$('select').trigger('refresh');}
if(step=="step3"){if($('.step2 input[name=lastname]').val()==""){$('.step2 input[name=lastname]').addClass('error');$('.step2 input[name=lastname]').attr('placeholder','Обязательно для заполнения');}
if($('.step2 input[name=name]').val()==""){$('.step2 input[name=name]').addClass('error');$('.step2 input[name=name]').attr('placeholder','Обязательно для заполнения');}
if($('.step2 input[name=phone]').val()==""){$('.step2 input[name=phone]').addClass('error');$('.step2 input[name=phone]').attr('placeholder','Обязательно для заполнения');}
if($('.step2 input[name=lastname]').val()!=""&&$('.step2 input[name=name]').val()!=""&&$('.step2 input[name=phone]').val()!=""){$('.steps li').removeClass('active');$('.step3li').addClass('active');$('.step2').css('display','none');$('.step3').css('display','block');}}}
